...
* **In:** Used for languages (`in English`), files (`in README.md`), or environments (`in production`).
* **On:** Used for branches (`on main`), platforms (`on GitHub`), or devices (`on mobile`).
* **At:** Used for precise targets (`at index 0`, `at line 15`).
* **By:** Used for methods (`by updating dependencies`).
* **With:** Used for tools (`built with Astro`).